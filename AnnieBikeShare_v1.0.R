...
pacman::p_load(dplyr, tidyr, lubridate, caret,randomForest, rpart.plot, rattle, party)
...
combi = bind_rows(train %>% mutate(datetime = as.character(datetime)), 
                  test %>% mutate(datetime = as.character(datetime))) # automatically fills missing columns with NA
head(combi)
summary(combi)
# Observe combi
str(combi)
...
# set seed to get the same random sample each time the code is run. 
...
#8PM - 12AM
combi$daypart[(combi$hour >= 20)] <- "4"
#12AM - 6AM
combi$daypart[(combi$hour < 6) ] <- "5"
...
ggplot(combi, aes(x = hour, y = count, col = registered)) +
  geom_point(size=3, alpha = 0.1, position = posn.jd)

ggplot(combi, aes(x = holiday, y = count, col = registered)) +
  geom_point(size=3, alpha = 0.1, position = posn.jd)

# Use dayparts for workingdays vs non-workingdays. See plot from above:
ggplot(combi, aes(x = hour, y = count, col = factor(workingday))) +
  geom_point(size=3, alpha = 0.1, position = posn.jd)

# Group by temperature. 
# Create a better plot to show weather-related variables
ggplot(train, aes(x = temp, y = count, col = factor(weather))) +
  geom_point(size=3, alpha = 0.1, position = posn.jd)
...
fit1 <- rpart(count ~ season + holiday + workingday + weather + workingday + weather + temp + atemp + humidity + windspeed + day + hour,
             data=train,
             method="anova")
...
rf1 <- randomForest(count ~ season + holiday + workingday + weather + workingday + weather + temp + atemp + humidity + windspeed + day + hour,
              data=train, 
              mtry = 3, 
              importance=TRUE, 
              ntree=20, do.trace = TRUE)
...
# create formula
fit <- cforest(count ~ season + holiday + workingday + weather + temp + atemp + humidity + windspeed + day + hour,
               data=train,
               mtry = 3, 
               importance=TRUE, 
               ntree=250, 
...
#write results to .csv for submission
write.csv(submit.ctree, file = "ctree_modelA.csv", row.names = FALSE)
